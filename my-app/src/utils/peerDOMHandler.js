...
export function addStream(isHost, stream, connUserSocketId, username, avatar) {
  ...
  const existingVideoElement = document.querySelector(`#video-${connUserSocketId}`);

  if (existingVideoElement) {
    console.log(`Stream for ${username} already exists.`);
    return; // Exit if the stream already exists
  }
